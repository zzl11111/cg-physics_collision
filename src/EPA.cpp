...
float signedDistanceToTriangle(const Triangle_t & triangle, const glm::vec3 p, const std::vector<Point_t> & points) {
...
// closestTriangle
int closestTriangle(const std::vector<class Triangle_t> & triangles, const std::vector<Point_t> & points) {
	float min_distance = 1e10;
	int min_index = -1;
	for (int i = 0; i < triangles.size(); i++) {
		float distance = signedDistanceToTriangle(triangles[i], points[0].xyz, points);
...
bool hasPoint(const glm::vec3 & w, const std::vector<Triangle_t> & triangles, const std::vector<Point_t> & points) {
	...
	for (int i = 0; i < triangles.size(); i++) {
		...
		if (glm::dot(delta, delta) < epsilon) { return true; }
...
void findDanglingEdges(std::vector<Edge_t> & danglingEdges, const std::vector<Triangle_t> & triangles) {
	...
	for (int i = 0; i < triangles.size(); i++) {
		...
		for (int j = 0; j < triangles.size(); j++) {
			...
			for (int k = 0; k < 3; k++) {
				if (edge[k] == other_edge[0]) { counts[k]++; }
				if (edge[k] == other_edge[1]) { counts[k]++; }
				if (edge[k] == other_edge[2]) { counts[k]++; }
			}
		}
		for (int k = 0; k < 3; k++) {
			if (counts[k] == 0) {
...
// EPA_expand
float EPA_expand(const Body * bodyA, const Body * bodyB, const float bias, std::vector<Point_t> & simplexPoints, glm::vec3 & ptOnA, glm::vec3 & ptOnB) {
	...
	for (int i = 0; i < 4; i++) {
		...
		int unusedPt = (i + 3) % 4;
		float dist = signedDistanceToTriangle(triangle, points[unusedPt].xyz, points);
	...
	while (1) {
		...
		if (hasPoint(newPoint.xyz, triangles, points)) { break; }

		float dist = signedDistanceToTriangle(triangles[index], newPoint.xyz, points);
		if (dist < 1e-6) { break; }
	...
	glm::vec3 lambdas = barycentricCoordinates(ptA_w, ptB_w, ptC_w, glm::vec3(0, 0, 0));
...
bool GJK_doesIntersect(const Body *bodyA, const Body *bodyB, const float bias, glm::vec3 &ptOnA, glm::vec3 &ptOnB) {
	...
	while (!doesContainOrigin) {
		...
		// If the noew point is the same as a previous point, then we can't expand any further
		...
		if (dotdot < 0.0f) { break; }

		glm::vec4 lambdas;
		doesContainOrigin = SimplexSignedVolumes(simplexPoints, numPoints, newDir, lambdas);
		...
		if (dist >= closetDist) { break; }
		closetDist = dist;

		// Use the lambdas that support thee new search direction, and invalidate any points that dont't support it	
		SortValids(simplexPoints, lambdas);
		numPoints = NumValids(lambdas);
		doesContainOrigin = numPoints == 4;
	}

	if (!doesContainOrigin) { return false; }
