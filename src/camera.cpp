...
Camera::Camera(glm::vec3 origin, glm::vec3 lookat, glm::vec3 up) {
    ...
    v = glm::normalize(v);
    if (camera_front.z <= 0) { yaw = acos(v.x); }
    else { yaw = 2 * PI - acos(v.x); }
...
void Camera::Process_camera_keyboard(unsigned int direction, float delta_time) {
    if (direction == (Direction::FRONT)) { camera_pos += camera_front * camera_speed * delta_time; }
    else if (direction == (Direction::BACK)) { camera_pos -= camera_front * camera_speed * delta_time; }
    else if (direction == (Direction::RIGHT)) { camera_pos += camera_right * camera_speed * delta_time; }
    else if (direction == (Direction::LEFT)) { camera_pos -= camera_right * camera_speed * delta_time; }
    else if (direction == (Direction::UP)) { camera_pos += camera_up * camera_speed * delta_time; }
    else if (direction == (Direction::DOWN)) { camera_pos -= camera_up * camera_speed * delta_time; }
}
