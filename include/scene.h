...
inline bool check_collision(Body &body1, Body &body2, Contact &contact) {
  ...
  contact.normal = normalize(
      position_between_body2_body1); // the collision point vector (b-a)
...
inline void Process_collision(Contact &contact) {
  ...
  glm::vec3 J =
      (1 + elasticity) *
      dot((B->m_linear_velocity - A->m_linear_velocity), contact.normal) *
      contact.normal /
      (A->m_inv_mass + B->m_inv_mass); // we only apply impulse through the
                                       // vector of contact_normal

  A->Process_Impulse(contact.A_potential_collision_point_world_space, J);
  B->Process_Impulse(contact.B_potential_collision_point_world_space, -J);
  ...
  contact.A_potential_collision_point_world_space; A->m_position += tA * ds;
  B->m_position -= tB * ds;*/
}
...
public:
  ...
  void Create_Scene() {
    int num_objs = 1;
    Body obj(new Sphere(1), glm::vec3(0, 0, 0), glm::vec3(0, 0, 0),
             glm::quat(1, 0, 0, 0), 1.0);
    obj.elasticity = 0.95;
    objs.push_back(obj);
    Body ground(new Sphere(100), glm::vec3(0, -103, 0), glm::vec3(0, 0, 0),
                glm::quat(1, 0, 0, 0), 0);
